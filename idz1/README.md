...
## Часть 6. Сравнение OLTP и OLAP

| Характеристика | OLTP (PostgreSQL) | OLAP (ClickHouse) |
|---|---|---|
| Модель хранения | Строковая модель хранения. Удобна для частых операций с отдельными строками. | Колоночная модель хранения. Удобна для чтения отдельных колонок на больших объёмах данных. |
| Типичный запрос | Получить заказ по `order_id`, обновить статус, добавить новый заказ. | Посчитать продажи по месяцам, построить отчёт по категориям, агрегировать большие объёмы данных. |
| Нормализация | Обычно используется нормализованная схема, чтобы уменьшить дублирование и поддерживать целостность данных. | Часто используются денормализованные таблицы, чтобы быстрее выполнять аналитические запросы без большого количества `JOIN`. |
| Транзакции | Есть полноценная поддержка транзакций: `BEGIN`, `COMMIT`, `ROLLBACK`. | Основной фокус не на транзакциях, а на быстрой аналитике и пакетной обработке данных. |
| Вставка | Подходит для частых небольших вставок и изменений данных. | Лучше подходит для пакетной загрузки больших объёмов данных. |
| Обновление/удаление | `UPDATE` и `DELETE` являются обычными операциями. | Обновления и удаления возможны, но обычно используются реже, чем вставка новых данных. |
| Масштабирование | Часто масштабируется вертикально: более мощный сервер, индексы, оптимизация запросов. | Хорошо подходит для горизонтального масштабирования и распределённой аналитики. |
| Типичный use case | Интернет-магазин, CRM, банковские операции, системы заказов. | Аналитика, отчёты, BI, хранение событий, анализ логов. |

В этой работе PostgreSQL использовался как OLTP-СУБД: данные были нормализованы до 3NF, были добавлены связи через внешние ключи, транзакции и индексы для точечных запросов.

Для аналитического сценария были использованы элементы денормализации: материализованное представление `mv_monthly_sales` и избыточное поле `product_name` в `order_items`. Это показывает, что для отчётов иногда удобнее заранее подготовить данные для чтения, даже если появляется дублирование.