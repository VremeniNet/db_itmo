...
Для демонстрации расширения кластера был добавлен третий шард с двумя репликами:

- `ch-s3-r1`;
- `ch-s3-r2`.

После этого в конфигурацию был добавлен новый кластер `cluster_3x2`.

Итоговая топология `cluster_3x2`:

| shard_num | replica_num | host_name | port |
|---:|---:|---|---:|
| 1 | 1 | `ch-s1-r1` | 9000 |
| 1 | 2 | `ch-s1-r2` | 9000 |
| 2 | 1 | `ch-s2-r1` | 9000 |
...
Для проверки использовался скрипт:

```text
scripts/reshard_demo.py
```
...
После добавления третьего шарда распределённая таблица `events_distributed` была пересоздана уже для кластера `cluster_3x2`.

Затем через `events_distributed` было вставлено:

```text
600000 новых строк
```

Общее количество строк после вставки:

| total_rows | new_rows |
|---:|---:|
| 2600000 | 600000 |

### Распределение старых и новых данных

| host | total_rows | new_rows | old_rows |
|---|---:|---:|---:|
| `ch-s1-r1` | 1201096 | 200488 | 1000608 |
| `ch-s1-r2` | 1201096 | 200488 | 1000608 |
| `ch-s2-r1` | 1199164 | 199772 | 999392 |
| `ch-s2-r2` | 1199164 | 199772 | 999392 |
| `ch-s3-r1` | 199740 | 199740 | 0 |
| `ch-s3-r2` | 199740 | 199740 | 0 |

Новые данные распределились уже по трём шардам:

| host | new_rows | unique_users |
|---|---:|---:|
| `ch-s1-r1` | 200488 | 100244 |
| `ch-s1-r2` | 200488 | 100244 |
| `ch-s2-r1` | 199772 | 99886 |
| `ch-s2-r2` | 199772 | 99886 |
| `ch-s3-r1` | 199740 | 99870 |
| `ch-s3-r2` | 199740 | 99870 |

Внутри каждого шарда реплики содержат одинаковое количество новых строк:

- шард 1: `ch-s1-r1 = ch-s1-r2 = 200488`;
- шард 2: `ch-s2-r1 = ch-s2-r2 = 199772`;
- шард 3: `ch-s3-r1 = ch-s3-r2 = 199740`.

### Что произошло со старыми данными

Старые данные автоматически не переехали на третий шард.

Проверка старых данных на третьем шарде:
...
Изменение конфигурации `Distributed`-таблицы влияет на маршрутизацию новых вставок, но не перемещает уже существующие данные.

Чтобы выполнить настоящую ребалансировку старых данных, нужно делать отдельную миграцию. Возможный подход:

1. создать новую локальную таблицу и новую `Distributed`-таблицу с новой схемой шардирования;
2. переложить старые данные через `INSERT INTO new_table SELECT ... FROM old_table`;
3. проверить распределение данных по шардам;
4. переключить чтение на новую таблицу;
5. удалить старую таблицу после проверки.

Итог: третий шард начал получать новые данные, но старые данные остались на первых двух шардах. Это показывает, что добавление шарда само по себе не выполняет автоматическую ребалансировку.