...
Сравнение сделано на основе результатов ИДЗ-1 и ИДЗ-2.

Важно учитывать, что объём данных отличается:

- в PostgreSQL использовалась нормализованная схема 3NF;
- в PostgreSQL было 1200 заказов и 3098 товарных позиций;
- в ClickHouse использовалась плоская денормализованная таблица `orders_flat`;
- в ClickHouse было загружено 1 000 000 строк.

Подробные результаты сравнения сохранены в файле:

```text
checks/pg_vs_ch_comparison.txt
````

| Запрос / операция         | PostgreSQL 3NF                                                      | ClickHouse flat                                                                     | Вывод                                                                                                                 |
| ------------------------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Вставка 1 строки          | `INSERT INTO orders`: 8.919 ms                                      | Данные загружались пакетно через `INSERT INTO ... SELECT FROM numbers(1000000)`     | PostgreSQL удобнее для одиночных транзакционных вставок, ClickHouse — для пакетной загрузки больших объёмов           |
| Топ-10 товаров            | 2.890 ms на 3098 строках `order_items`                              | 0.691 sec на 1 000 000 строках `orders_flat`                                        | PostgreSQL быстро обработал маленький нормализованный набор, ClickHouse обработал намного больший аналитический набор |
| JOIN 4 таблиц             | `orders + customers + order_items + products`: 0.150 ms             | JOIN не нужен, данные заказа, клиента, товара и категории уже лежат в `orders_flat` | В PostgreSQL JOIN естественен для 3NF, в ClickHouse JOIN часто убирают за счёт денормализации                         |
| Обновление статуса        | `UPDATE orders SET status = 'shipped' WHERE order_id = 1`: 1.670 ms | Частые точечные обновления не являются типичным сценарием для ClickHouse            | PostgreSQL лучше подходит для OLTP-обновлений, ClickHouse — для append-only аналитики                                 |
| Размер на диске           | база `idz1`: 11 MB, самая большая таблица `order_items`: 624 kB     | `orders_flat`: 13.16 MiB на 1 000 000 строк                                         | ClickHouse компактно хранит большой плоский набор за счёт колоночного хранения, сжатия, `LowCardinality` и сортировки |
| Поиск по подстроке        | `ILIKE '%Иван%'`: 0.278 ms на таблице `customers` из 10 строк       | `positionCaseInsensitive(customer_email, 'ivanov')`: 0.115 sec на 1 000 000 строках | PostgreSQL быстрее на маленькой таблице, ClickHouse быстро обрабатывает большой плоский набор                         |
| Предрассчитанные агрегаты | В ИДЗ-1 использовалось materialized view `mv_monthly_sales`         | `orders_flat`: 0.213 sec, `monthly_sales`: 0.014 sec                                | Предрассчитанные агрегаты ускоряют аналитическое чтение                                                               |

### Вывод

PostgreSQL в ИДЗ-1 использовался как OLTP-СУБД.
Для него важны нормализация, транзакции, внешние ключи, точечные вставки, обновления и согласованность данных.

ClickHouse в ИДЗ-2 использовался как OLAP-СУБД.
Для него важны большие объёмы данных, колоночное хранение, сжатие, быстрые агрегации и денормализация.

Главное различие видно по структуре данных:

* в PostgreSQL данные были разделены на таблицы `customers`, `addresses`, `categories`, `products`, `orders`, `order_items`;
* в ClickHouse данные были собраны в плоскую таблицу `orders_flat`.

Для операционной части интернет-магазина удобнее PostgreSQL.
Для аналитики и отчётов по большому объёму строк удобнее ClickHouse.